...
{
    NeighborArrayUnitReset(pictureControlSetPtr->epIntraLumaModeNeighborArray);
    NeighborArrayUnitReset(pictureControlSetPtr->epMvNeighborArray);
    NeighborArrayUnitReset(pictureControlSetPtr->epSkipFlagNeighborArray);
    NeighborArrayUnitReset(pictureControlSetPtr->epModeTypeNeighborArray);
    NeighborArrayUnitReset(pictureControlSetPtr->epLeafDepthNeighborArray);
    NeighborArrayUnitReset(pictureControlSetPtr->epLumaReconNeighborArray);
    NeighborArrayUnitReset(pictureControlSetPtr->epCbReconNeighborArray);
    NeighborArrayUnitReset(pictureControlSetPtr->epCrReconNeighborArray);
    NeighborArrayUnitReset(pictureControlSetPtr->epSaoNeighborArray);
    NeighborArrayUnitReset(pictureControlSetPtr->amvpMvMergeMvNeighborArray);
    NeighborArrayUnitReset(pictureControlSetPtr->amvpMvMergeModeTypeNeighborArray);
    ...
    EncDecContext_t                        *contextPtr)
{
    ...
    for (cuDepth = 0; cuDepth < 4; cuDepth++){
        contextPtr->minDeltaQp[cuDepth] = pictureControlSetPtr->sliceType == I_SLICE ? encMinDeltaQpISliceTab[cuDepth] : encMinDeltaQpTab[cuDepth][pictureControlSetPtr->temporalLayerIndex];
...
EB_ERRORTYPE SignalDerivationEncDecKernelSq(
    ...
    if (pictureControlSetPtr->encMode == ENC_MODE_0) {
        ...
        if (sequenceControlSetPtr->inputResolution < INPUT_SIZE_4K_RANGE) {
            ...
            contextPtr->mdContext->intraMdOpenLoopFlag = pictureControlSetPtr->sliceType == I_SLICE ? EB_FALSE : EB_TRUE;
        }
    }
    else {
        contextPtr->mdContext->intraMdOpenLoopFlag = pictureControlSetPtr->sliceType == I_SLICE ? EB_FALSE : EB_TRUE;
    ...
    if (pictureControlSetPtr->ParentPcsPtr->encMode <= ENC_MODE_4) {          
        if (pictureControlSetPtr->sliceType == I_SLICE && contextPtr->mdContext->intraMdOpenLoopFlag == EB_FALSE) {
...
EB_ERRORTYPE SignalDerivationEncDecKernelOq(
    ...
    // Set Allow EncDec Mismatch Flag
    if (pictureControlSetPtr->ParentPcsPtr->encMode <= ENC_MODE_7) {
        contextPtr->allowEncDecMismatch = EB_FALSE;
    }
    ...
    else if (sequenceControlSetPtr->inputResolution > INPUT_SIZE_576p_RANGE_OR_LOWER) {
    ...
    // Set RDOQ/PM_CORE Flag
    if (sequenceControlSetPtr->inputResolution == INPUT_SIZE_4K_RANGE) {
        contextPtr->mdContext->rdoqPmCoreMethod =(pictureControlSetPtr->ParentPcsPtr->encMode <= ENC_MODE_1) ?
            EB_RDOQ :
            (pictureControlSetPtr->ParentPcsPtr->encMode <= ENC_MODE_5) ?
                EB_PMCORE :
                EB_NO_RDOQ;
    }
    else {
        contextPtr->mdContext->rdoqPmCoreMethod = (pictureControlSetPtr->ParentPcsPtr->encMode == ENC_MODE_0) ?
            EB_RDOQ :
            (pictureControlSetPtr->ParentPcsPtr->encMode <= ENC_MODE_3) ?
                EB_PMCORE :
                EB_NO_RDOQ;
    }
    ...
    if (pictureControlSetPtr->encMode == ENC_MODE_0) {
        ...
        if (sequenceControlSetPtr->inputResolution <= INPUT_SIZE_576p_RANGE_OR_LOWER || sequenceControlSetPtr->inputResolution == INPUT_SIZE_1080p_RANGE) {
            contextPtr->mdContext->mpmLevel = 0;
        }
        else {
            contextPtr->mdContext->mpmLevel = 2;
    ...
    if (pictureControlSetPtr->encMode <= ENC_MODE_3) {
        ...
        if (sequenceControlSetPtr->inputResolution < INPUT_SIZE_4K_RANGE) {
            if (pictureControlSetPtr->temporalLayerIndex == 0) {
                ...
                contextPtr->mdContext->nmmLevelMd = 2;
            }
    ...
    if (pictureControlSetPtr->encMode == ENC_MODE_0) {
        ...
        if (sequenceControlSetPtr->inputResolution == INPUT_SIZE_1080p_RANGE || sequenceControlSetPtr->inputResolution == INPUT_SIZE_1080i_RANGE) {
            if (pictureControlSetPtr->sliceType == I_SLICE) {
                contextPtr->mdContext->nflLevelMd = 0;
            }
            else {
...
void* EncDecKernel(void *inputPtr)
{
    ...
    for (;;) {
        ...
        while (AssignEncDecSegments(segmentsPtr, &segmentIndex, encDecTasksPtr, contextPtr->encDecFeedbackFifoPtr) == EB_TRUE)
        {
            ...
            lcuStartIndex = yLcuStartIndex * pictureWidthInLcu + xLcuStartIndex;
            ...
            for (yLcuIndex = yLcuStartIndex, lcuSegmentIndex = lcuStartIndex; lcuSegmentIndex < lcuStartIndex + lcuSegmentCount; ++yLcuIndex) {
                for (xLcuIndex = xLcuStartIndex; xLcuIndex < pictureWidthInLcu && (xLcuIndex + yLcuIndex < segmentBandSize) && lcuSegmentIndex < lcuStartIndex + lcuSegmentCount; ++xLcuIndex, ++lcuSegmentIndex) {
                    ...
                    endOfRowFlag = (xLcuIndex == pictureWidthInLcu - 1) ? EB_TRUE : EB_FALSE;
                    lcuRowIndexStart = (xLcuIndex == pictureWidthInLcu - 1 && lcuRowIndexCount == 0) ? yLcuIndex : lcuRowIndexStart;
                    ...
                    if ( 
                        ...
                        if (pictureControlSetPtr->sliceType != I_SLICE && isFourCu32x32) {
